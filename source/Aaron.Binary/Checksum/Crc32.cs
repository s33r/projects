...
// Reference: https://github.com/Michaelangel007/crc32
...
namespace Aaron.Binary.Checksum
{
    ...
    public static class Crc32
    {
                                   ...
                                   uint sum)
        {
            ...
            for (int i = offset; i < offset + length; i++) { newSum = table[(newSum >> 24) ^ data[i]] ^ (newSum << 8); }
        ...
        public static uint Compute(Crc32StandardPolynomials polynomial, string data, uint sum)
        {
            ...
            foreach (char t in data) { newSum = table[(newSum >> 24) ^ t] ^ (newSum << 8); }

            return ~newSum;
        }

        public static uint Compute(Crc32StandardPolynomials polynomial, byte[] data)
        {
            return Compute(polynomial, data, 0, 0, 0);
        }
        ...
        public static uint Compute(Crc32StandardPolynomials polynomial, byte[] data, int offset, int length)
        {
            return Compute(polynomial, data, offset, length, 0);
        }

        public static uint Compute(byte[] data)
        {
            return Compute(Crc32StandardPolynomials.Normal, data, 0, 0, 0);
        }
        ...
        {
            return Compute(Crc32StandardPolynomials.Normal, data, offset, length, 0);
        }

        public static uint Compute(Crc32StandardPolynomials polynomial, string data)
        {
            return Compute(polynomial, data, 0);
        }

        public static uint Compute(string data)
        {
            return Compute(Crc32StandardPolynomials.Normal, data, 0);
        }
    }
}